...
export default function MyOrdersPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "pending":
                return (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-[10px] font-bold uppercase tracking-wider">
                        <LuClock size={10} />
                        Pendiente
                    </span>
                );
            ...
            case "failed":
                return (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-100 text-red-700 rounded-full text-[10px] font-bold uppercase tracking-wider">
                        <LuInfo size={10} />
                        Fallido
                    </span>
                );
            ...
            default:
                return (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[var(--background)] text-[var(--foreground)] rounded-full text-[10px] font-bold uppercase tracking-wider">
                        {status}
                    </span>
                );
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] pt-32 pb-20">
            <div className="max-w-5xl mx-auto px-6">
                ...
                {error && (
                    <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 flex items-center gap-3">
                        <LuInfo />
                        {error}
                    </div>
                )}
                ...
                {orders.length === 0 ? (
                    <div className="bg-[var(--card)] rounded-3xl p-16 text-center shadow-sm border border-[var(--border)] animate-in fade-in zoom-in duration-500">
                    ...
                    <div className="space-y-8">
                        {orders.map((order, index) => (
                            <div
                                key={order.id}
                                className="bg-[var(--card)] rounded-3xl overflow-hidden shadow-sm border border-[var(--border)] hover:shadow-md transition-all animate-in fade-in slide-in-from-bottom-4 duration-500"
                                style={{ animationDelay: `${index * 100}ms` }}
                            >
                                {/* Header de la Orden */}
                                <div className="bg-[var(--background)] p-6 lg:p-8 border-b border-[var(--border)] flex flex-wrap justify-between items-center gap-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-[var(--card)] rounded-2xl flex items-center justify-center text-blue-600 shadow-sm border border-[var(--border)]">
                                            <LuPackageOpen size={20} />
                                        </div>

                                        <div>
                                            <p className="text-[10px] text-[var(--foreground)]/30 uppercase font-black tracking-widest mb-0.5">Pedido</p>
                                    ...
                                    <div className="flex flex-wrap items-center gap-8">
                                        <div className="hidden sm:block">
                                            <p className="text-[10px] text-[var(--foreground)]/30 uppercase font-black tracking-widest mb-0.5">Fecha</p>
                                            <div className="flex items-center gap-2 text-sm font-bold text-[var(--foreground)]">
                                                <LuCalendar size={12} className="text-[var(--foreground)]/30" />
                                                {new Date(order.created_at).toLocaleDateString()}
                                            </div>
                                        </div>

                                        <div>
                                            <p className="text-[10px] text-[var(--foreground)]/30 uppercase font-black tracking-widest mb-0.5">Total</p>
                                            <p className="text-xl font-black text-[var(--foreground)]">${order.total_amount.toLocaleString('es-AR')}</p>
                                        </div>

                                        <div className="flex flex-col items-end">
                                            <p className="text-[10px] text-[var(--foreground)]/30 uppercase font-black tracking-widest mb-1">Estado</p>
                                    ...
                                    <div className="space-y-6">
                                        {order.items.map((item) => (
                                            <div key={item.id} className="flex items-center gap-6 group">
                                                <div className="w-24 h-24 bg-[var(--background)] rounded-2xl overflow-hidden flex-shrink-0 border border-[var(--border)] group-hover:scale-105 transition-transform">
                                                ...
                                                <div className="flex-grow">
                                                    ...
                                                    <div className="flex items-center gap-4 text-sm text-[var(--foreground)]/50">
                                                        ...
                                                        <span>•</span>
                                                        <span className="font-mono font-bold text-[var(--foreground)]/70">
